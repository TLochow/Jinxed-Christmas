...
func GetEvaluatorForPresent(present, difficulty):
	...
	var valid = false
	while not valid:
		mainEvaluator = EVALUATORSCENE.instance()
		mainEvaluator.HeadEvaluator = true
		...
		valid = mainEvaluator.Evaluate(present)
		if not valid:
			var presents = get_tree().get_nodes_in_group("Present")
			for currentPresent in presents:
				if not valid:
					valid = mainEvaluator.Evaluate(currentPresent)
			if not valid:
				mainEvaluator.queue_free()
